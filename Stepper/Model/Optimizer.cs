...
namespace Stepper
{
    public class Optimizer
    {
        ...
        public Optimizer(StepperDesign design)
        {
            this.Design = design;
        ...
        public void Iterate(List<List<double>> DesignMap)
        {
            ...
            System.Action run = delegate ()
            {
                foreach (List<double> sample in DesignMap)
                {
                    ...
                    {
                        Design.Variables[i].CurrentValue = val;
                        i++;
                    }
        ...
        public void Optimize()
        {
            ...
            for (int i = 0; i < numVars; i++)
            {
                ...
                if (Design.MyComponent.Direction > 0)
                {
                    double SteppedValue = var.CurrentValue + Gradient[Design.MyComponent.ObjIndex][i] * Design.MyComponent.StepSize * (var.Max - var.Min);
                ...
                if (Design.MyComponent.Direction == 0)
                {
                    double SteppedValue = var.CurrentValue + IsoPerfDirList[i] * Design.MyComponent.StepSize * numVars;
                    var.CurrentValue = SteppedValue;
                }
            }
        }
